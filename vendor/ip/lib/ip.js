...
// NOTE: ip.isPrivate / ip.isPublic have been intentionally removed from this
// vendored copy. Those helpers carry the SSRF misclassification advisory
// GHSA-2p57-rm9w-gvfp and are not used by werift (which only needs toBuffer /
// toString / isV4Format / isLoopback). See README of webrtctesttool.
...
ip.address = function (name, family) {
  ...
  // The 'public' / 'private' interface filter relied on the removed
  // isPrivate/isPublic helpers and is unsupported in this vendored copy.
  if (name === 'public' || name === 'private') {
    throw new Error("ip.address('public'|'private') is not supported in this vendored ip");
  }

  const all = Object.keys(interfaces).map((nic) => {
    const addresses = interfaces[nic].filter((details) => {
      details.family = _normalizeFamily(details.family);
      return details.family === family && !ip.isLoopback(details.address);
    });
