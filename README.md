...
| 只有 1 個 STUN 伺服器確實回應 | 樣本不足，需要 ≥2 個 |
| ≥2 個伺服器回應，對外 port 一致 | Cone-type NAT（可預測位址轉換） |
| ≥2 個伺服器回應，對外 port 不同 | Symmetric NAT（位址相依轉換） |

「≥2 個伺服器確實回應」用的是 `RTCPeerConnection` 的 `icecandidateerror` 事件去算，
**不是**單純數 srflx candidate 的數量——因為當兩個 STUN 伺服器回傳*相同*的對外
位址時（典型的 Cone NAT 現象），ICE candidate 去重（RFC 8445 §5.1.3）會在
`getStats()` 看到之前就先把其中一個判定為重複而丟棄，只看剩幾個 candidate 沒辦法
分辨「兩個伺服器答案一致」跟「其中一個根本沒回應」。

### 已知限制

- 若某個設定的 STUN 伺服器是「封包送出去沒有任何回應」（逾時而非快速的
  connection-refused/DNS 失敗），瀏覽器可能整批放棄該次 STUN 收集、不等它，
  此時會直接顯示「無法判斷」——這是瀏覽器 ICE gathering 的行為，應用層無法繞過。
  真實世界的公用 STUN 供應商故障通常是快速失敗（DNS 或 connection-refused），
  這個情境較少見。
