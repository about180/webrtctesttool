...
按「**執行網路診斷**」（跟「開始測試」分開、不需選時長，幾秒內出結果；若連線已經
建立會直接重用，不重新握手）可以看到 WebRTC 連線建立過程的完整細節，用於評估
「這個網路環境到底建不建得起來 WebRTC」以及日後排錯：

- **NAT Type**（近似判斷）
- **STUN Binding**：每個 STUN 伺服器回報的對外 IP:Port
- **Local / Remote Candidates**：完整的 ICE candidate 清單（type、protocol、
  address、port、priority、foundation、related address/port、來源 STUN/TURN
  伺服器）
- **Candidate Pairs**：本次嘗試過的每一組配對（不只贏家），標出目前使用中
  （`succeeded` / nominated）的那組，含 local/remote 的 ip:port 與 RTT

資料完全來自瀏覽器原生 `RTCPeerConnection.getStats()`，沒有額外相依套件。

### NAT Type 是近似判斷，不是 RFC 3489 完整分類

真正的 Full/Restricted/Port-Restricted Cone vs Symmetric 分類需要 STUN 伺服器支援
CHANGE-REQUEST（RFC 5780），但公用 STUN 伺服器都已不支援，瀏覽器也沒有 raw socket
可以自己發封包做完整偵測。這裡改用比對 **≥2 個不同 STUN 伺服器**回傳的對外 port
是否一致的近似做法：

| 觀察結果 | 判斷 |
|---------|------|
| 未設定 STUN（`LAN_ONLY=1`） | N/A |
| 有設定但完全沒有 STUN 回應 | 無法判斷（逾時或 UDP 被封鎖） |
| 只有 1 個 STUN 伺服器回應（其餘逾時） | 樣本不足，需要 ≥2 個 |
| ≥2 個伺服器參與、對外 port 一致 | Cone-type NAT（可預測位址轉換） |
| ≥2 個伺服器回應、對外 port 不同 | Symmetric NAT（位址相依轉換） |

實作上有兩個關鍵細節：

1. **用一條專門的「純收集」連線去打 STUN**，而不是沿用吞吐量測試那條主連線。主連線在
   區網上會用 host-host candidate **瞬間連上**，瀏覽器接著就會**縮減 STUN 收集**，導致
   從主連線讀到的 srflx 不完整。純收集連線不連任何對端，收集會跑完、確實去問每一台
   STUN；而且所有 STUN 都從**同一個本機通訊埠**發出，這正是讓「對外 port 比對」有效的
   前提。
2. **對外 port 相同時 ICE 會去重**（RFC 8445 §5.1.3）：兩台 STUN 回傳相同映射（Cone
   NAT 的典型現象）時，其中一筆 candidate 會在 `getStats()` 之前就被判為重複丟棄，
   所以上表 STUN Binding 常會是「成功 1 / 共 2」、另一台標為「無 srflx（可能被去重）」。
   只要那台沒有明確逾時（`icecandidateerror` errorCode ≥ 700），就視為它有參與 →
   判為 Cone。反之若它明確逾時，則只有 1 台真正回應 → 樣本不足。

### 已知限制

- 前述「對外 port 相同時去重」讓「只有 1 筆 srflx」這個情況本質上帶點模糊：它可能是
  Cone（兩台答案一致被去重），也可能是「第二台其實沒回應但也沒報錯」。本工具的預設是
  **偏向判為 Cone**（因為真實網路裡故障的 STUN 幾乎都會觸發 `icecandidateerror`，
  「沒 srflx 又沒報錯」絕大多數是去重）。每台 STUN 的實際狀態都列在 STUN Binding
  表格裡，可自行核對。
- 現代瀏覽器預設會用 mDNS 隱藏本機真實區網 IP（隱私保護），因此 Local Candidates
  的 host 位址欄位常顯示為空，屬正常現象、無法從網頁 JS 取得真實內網 IP。
- 「公網 IP / 無 NAT」的判斷依賴比對 srflx 位址與 host 位址是否相同；上述 mDNS
  遮蔽會讓這個比對幾乎不會觸發，實務上此情境多半會落入 Cone-type 分類。

### 讓 NAT Type 有東西可比對：預設多個 STUN 伺服器

`server/config.js` 非 `LAN_ONLY` 時預設用以下公用 STUN，可用環境變數覆蓋：
...
> 注意：`stun.l.google.com` 的各個埠都解析到**同一個 IP**，對 Symmetric vs Cone 的
> 比對來說，真正有鑑別力的第二個資料點是 **Cloudflare（不同 IP）**。

| 變數 | 說明 |
|------|------|
| `STUN_URLS` | 逗號分隔的 STUN 網址清單，優先套用 |
| `STUN_URL` | 舊有單一值（向下相容）——只設這個的話 NAT Type 會顯示「樣本不足」 |
...
│   ├── diagnostics.js # ICE / STUN / NAT 網路診斷（getStats() 解析、NAT type 判斷）
│   └── styles.css
├── server/
